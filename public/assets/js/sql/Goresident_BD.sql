-- ============================================================
-- GoResidentGo - Base de datos MySQL/MariaDB compatible con phpMyAdmin
-- Proyecto original: HTML + JS con localStorage
-- Esta versión migra la persistencia a MySQL y agrega triggers.
-- Probado a nivel de sintaxis lógica para MySQL 8 / MariaDB 10.4+
-- ============================================================
...
-- EJEMPLOS RAPIDOS
-- ============================================================
-- Registrar ENTRADA residente:
-- INSERT INTO parking_sessions(plate, vehicle_type, access_type, unit_name, space_id, resident_id, entry_by_user_id)
-- VALUES ('ABC123', 'Carro', 'Residente', 'Torre 1 - Apto 302',
--         (SELECT id FROM parking_spaces WHERE code = 'A-01'),
--         (SELECT id FROM residents WHERE plate = 'ABC123'),
--         (SELECT id FROM users WHERE email = 'porteria@goresidentgo.com'));
--
-- Registrar ENTRADA visitante:
-- INSERT INTO parking_sessions(
--   plate, vehicle_type, access_type, unit_name, space_id,
--   authorized_by_resident_id, authorized_ok, visitor_name, visitor_document, visitor_notes, entry_by_user_id
-- ) VALUES (
--   'VIS123', 'Carro', 'Visitante', 'Torre 1 - Apto 302',
--   (SELECT id FROM parking_spaces WHERE code = 'A-26'),
--   (SELECT id FROM residents WHERE plate = 'ABC123'), 1,
--   'Juan Perez', 'CC123456', 'Visita familiar',
--   (SELECT id FROM users WHERE email = 'porteria@goresidentgo.com')
-- );
--
-- Registrar SALIDA:
...
--        exit_by_user_id = (SELECT id FROM users WHERE email = 'porteria@goresidentgo.com')
--  WHERE ticket = 'T-1001';
